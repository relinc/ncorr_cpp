					...
					if ((p1 > 0 && p1 < A_inv_loc.height() - 1) || (p2 > 0 && p2 < A_inv_loc.width() - 1)) {
						...
						if (p1 == 0 || p1 == A_inv_loc.height() - 1) {
							...
							if (A_inv_loc(p1, p2 - 1) != -1 || A_inv_loc(p1, p2) != -1 || A_inv_loc(p1, p2 + 1) != -1) {
								...
								b.push_back(0);
								if (A_inv_loc(p1, p2 - 1) == -1) { b[b.size() - 1] -= A(p1, p2 - 1); }
								else { add_to_sparse_tree(sparse_tree, { difference_type(b.size()) - 1, A_inv_loc(p1,p2 - 1), 1 }); }
								if (A_inv_loc(p1, p2) == -1) { b[b.size() - 1] += 2 * A(p1, p2); }
								else { add_to_sparse_tree(sparse_tree, { difference_type(b.size()) - 1, A_inv_loc(p1,p2),  -2 }); }
								if (A_inv_loc(p1, p2 + 1) == -1) { b[b.size() - 1] -= A(p1, p2 + 1); }
								else { add_to_sparse_tree(sparse_tree, { difference_type(b.size()) - 1, A_inv_loc(p1,p2 + 1), 1 }); }
							}
						}
							...
							if (A_inv_loc(p1 - 1, p2) != -1 || A_inv_loc(p1, p2) != -1 || A_inv_loc(p1 + 1, p2) != -1) {
								// Point of interest - add a constraint
								b.push_back(0);
								if (A_inv_loc(p1 - 1, p2) == -1) { b[b.size() - 1] -= A(p1 - 1, p2); }
								else { add_to_sparse_tree(sparse_tree, { difference_type(b.size()) - 1, A_inv_loc(p1 - 1,p2), 1 }); }
								if (A_inv_loc(p1, p2) == -1) { b[b.size() - 1] += 2 * A(p1, p2); }
								else { add_to_sparse_tree(sparse_tree, { difference_type(b.size()) - 1, A_inv_loc(p1,p2),  -2 }); }
								if (A_inv_loc(p1 + 1, p2) == -1) { b[b.size() - 1] -= A(p1 + 1, p2); }
								else { add_to_sparse_tree(sparse_tree, { difference_type(b.size()) - 1, A_inv_loc(p1 + 1,p2), 1 }); }
							...
							if (A_inv_loc(p1 - 1, p2) != -1 || A_inv_loc(p1 + 1, p2) != -1 || A_inv_loc(p1, p2) != -1 || A_inv_loc(p1, p2 - 1) != -1 || A_inv_loc(p1, p2 + 1) != -1) {
								...
								if (A_inv_loc(p1, p2) == -1) { b[b.size() - 1] += 4 * A(p1, p2); }
								else { add_to_sparse_tree(sparse_tree, { difference_type(b.size()) - 1, A_inv_loc(p1,p2),  -4 }); }
								if (A_inv_loc(p1, p2 - 1) == -1) { b[b.size() - 1] -= A(p1, p2 - 1); }
								else { add_to_sparse_tree(sparse_tree, { difference_type(b.size()) - 1, A_inv_loc(p1,p2 - 1), 1 }); }
								if (A_inv_loc(p1, p2 + 1) == -1) { b[b.size() - 1] -= A(p1, p2 + 1); }
								else { add_to_sparse_tree(sparse_tree, { difference_type(b.size()) - 1, A_inv_loc(p1,p2 + 1), 1 }); }
