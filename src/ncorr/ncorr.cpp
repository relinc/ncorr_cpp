...
namespace ncorr {
		...
		void worker_RGDIC(subregion_nloptimizer sr_nloptimizer,     // passed by value
			...
			for (difference_type region_idx = 0; region_idx < roi_reduced.size_regions(); ++region_idx) {
				...
				auto seed_params = get_seed_params(redundant_seeds[region_idx],
					sr_nloptimizer,
					params_buf);
				//std::cout<<seed_params.size_2D_string()<<std::endl;
				if (!seed_params.empty()) {
	...
	std::pair<Disp2D, Data2D> RGDIC(const Array2D<double> &A_ref,
		...
		if (debug) {
			...
			for (const auto &subset_seed : subset_seeds) {
				...
				// Display subset
				std::cout << "display subset" << std::endl;
				imshow(subset, 2000);
			}

			// Display partition diagram
			std::cout << "display partition" << std::endl;
	...
	DIC_analysis_output DIC_analysis(const DIC_analysis_input &DIC_input) {
		...
		std::cout << "this file just got edited" << std::endl;
	...
	//Strain2D&  calculate_e1_e2(Strain2D& strainData){
	//	int height =strainData.get_exx().data_height();
	//	int width =strainData.get_exx().data_width();
	//	Array2D<double> e1array(height,width);
	//	Array2D<double> e2array(height,width);
	//	for(int idy=0; idy<height;idy++){
	//		for(int idx=0;width;idx++){
	//			auto exx=strainData.get_exx().get_array()(idy,idx);
	//			auto eyy =strainData.get_eyy().get_array()(idy,idx);
	//			auto exy =strainData.get_exy().get_array()(idy,idx);
	//			double trace = exx+eyy;
	//			double det = exx*eyy-exy*exy;
	//			e1array(idy,idx)=std::max((trace-std::sqrt(trace*trace -4*det))/2,
	//				(trace+std::sqrt(trace*trace -4*det))/2);
	//			e2array(idy,idx)=std::min((trace-std::sqrt(trace*trace -4*det))/2,
	//				(trace+std::sqrt(trace*trace -4*det))/2);
	//		}
	//	}
	//	return Strain2D(strainData.get_eyy().get_array(),strainData.get_exy().get_array(),strainData.get_exx().get_array(),e1array,e2array,strainData.get_roi(),strainData.get_scalefactor());
	//
	//}
