...
namespace ncorr {
	...
	public:
		...
		friend Array2D pad(const Array2D &A, difference_type padding, PAD pad_type = static_cast<PAD>(0)) { return A.this_pad(padding, pad_type); }
		...
		// Utility -----------------------------------------------------------//
		bool empty() const { std::cout << s << std::endl; return s == 0; }
		template <typename T_container>
	...
	namespace details {
		...
		public:
			...
			template<typename T_container2>
			base_region&
				operator=(const base_region<T_container2>&);
	...
	namespace details {
		...
		// Simple Iterator -------------------------------------------------------//
		template <typename T_container>
		inline typename simple_iterator<T_container>& simple_iterator<T_container>::operator++() {
		...
		template <typename T_container>
		template <typename T_container2>
		base_region< T_container>& base_region<typename T_container>::operator=(const base_region<T_container2> &reg) {
			if (this->region_h != reg.region_h || this->region_w != reg.region_w) {
				throw std::invalid_argument("Attempted to assign region of size: " + reg.region_size_2D_string() +
					" to region of size: " + region_size_2D_string() + ".");
			}
